...
//Funzione che permette di trasformare i millisecondi in secondi : minuti
    ...
    if (star.length==0){
        document.getElementById('tabella').innerHTML='<tr><td><h1 style="margin:0 auto; font-weight:500; padding:20px; font-size:18px;">Nessun utente ha pubblicato la sua playlist per il momento</h1></td></tr>'
    }else{
        liste()
    }
    ...
    //Ricerca delle playlist tramite tag
        ...
        if(d==''){
            document.getElementById('esperimento').innerHTML='<tr><td colspan="4"><br><h1 style="padding:0; margin: 0 auto; font-size:16px;">inserisci tag di ricerca</h1></td></tr><br>'
            liste();
        }else{
    ...
    //Mostra le playlist
        ...
        for(let i =0;i<star.length; i++){
            ...
            if(star[i].nome==nomeplaylist && star[i].utente==utente){
                ...
                tab.innerHTML+= '<tr id= "'+star[i].nome+'"  class="playlist"><td class="nomePlaylist" style="margin-top:30px;border-top-left-radius:25px; border-bottom-left-radius:25px;" title="Mostra Playlist" onclick = "mostra(\'' + star[i].nome + '\',\'' + star[i].utente + '\')" >'
                + star[i].nome+'<p style="margin-top:0px;;padding:0;font-size:14px;">di '+star[i].utente+'</p></td><td class="iconaPlaylist" style="text-align:center;border-top-right-radius:25px; border-bottom-right-radius:25px;"><img src="import.png" style=" margin-top:15px; height:25px; float:center;" class="cuore" title="Importa Playlist" id="importa'+star[i].nome+'" onclick="importa(\'' + star[i].nome + '\',\'' + star[i].utente + '\')"><p style="float:center; text-align:center;font-size:12px; margin-top:45px; margin-right:20px;">importa</p></td></tr>'
                + '<tr><td colspan="2" >'
                + '<table id ="tags'+star[i].nome+'" class="canzoni"></table></td></tr>'
                + '<tr><td colspan="2" ><table id ="brani'+star[i].nome+'" class="canzoni"></table></td></tr>';
    ...
    //Permette di importare le playlist
        ...
        if(document.getElementById('importa'+nomepl).src!="imported.png"){
            document.getElementById('importa'+nomepl).src="imported.png"
        }
