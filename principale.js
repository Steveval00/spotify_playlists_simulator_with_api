...
//Funzione che permette di trasformare i millisecondi in secondi : minuti  
 ...
 localStorage.setItem('token',tokenResponse.access_token)
      ...
      if(l[i]!=null){
        ...
        if(l[i].utente==sessname){
          ...
          if(l[i].pubblica==false){
            t.innerHTML+= '<tr id= "'+l[i].nome+ '" class="playlist"><td class="addtag"><img style="height:24px; margin-left:20px;" title="Aggiungi Tag" src="addtag.png" class="cuore" onclick = "mostraAggiuntaTag(\'' + l[i].nome + '\')"></td><td class="nomePlaylist" title="Mostra Playlist" onclick = "mostra(\'' + l[i].nome + '\')" " >'+l[i].nome+'</td><td class="iconaPlaylist"><img src="addsongs.png" class="cuore" title="Aggiungi Canzoni" id="'+l[i].nome+'" onclick="modifica(\'' + l[i].nome + '\')"></td><td class="iconaPlaylist"><label class="switch"  id="global'+ l[i].nome+'"  onclick="pubPriv(\'' + l[i].nome + '\')"><input type="checkbox" unchecked><span class="slider round"></span><p style="font-size:12px; margin-top:25px;">pubblica</p></label></td><td style="border-top-right-radius:25px; border-bottom-right-radius:25px;" class="iconaPlaylist"><img src="delete.png" class="cuore" title="Cancella Playlist" id="cancella'+l[i].nome+ '" onclick="cancellaplaylist(\'' + l[i].nome + '\')"></td></tr>'
            + '<tr><td colspan="5" >'
            + '<div id="aggiungiTag'+l[i].nome+ '"  style=" background-color:white; display:none; margin:0 auto; margin-bottom: 10px; width:800px;">'
            + '<input type="text" class="insTag" id="insTag'+l[i].nome+'" placeholder="#tag">'
            + '<input type="button" class="nascondi" style="float:right; margin-right:32%; height:40px; width:40px; padding:0px;" value="+" onclick="tags(\'' +l[i].nome+ '\')"> </div>'
            + '<table id ="tags'+l[i].nome+ '" class="canzoni"></table></td></tr>'
            + '<tr><td colspan="5" ><table id ="brani'+l[i].nome+ '" class="canzoni"></table></td></tr>';
          }else {
            t.innerHTML+= '<tr id= "'+l[i].nome+ '" class="playlist"><td class="addtag"><img style="height:24px; margin-left:20px;" title="Aggiungi Tag" src="addtag.png" class="cuore" onclick = "mostraAggiuntaTag(\'' + l[i].nome + '\')"></td><td class="nomePlaylist" title="Mostra Playlist" onclick = "mostra(\'' + l[i].nome + '\')" " >'+l[i].nome+'</td><td class="iconaPlaylist"><img src="addsongs.png" class="cuore" title="Aggiungi Canzoni" id="'+l[i].nome+'" onclick="modifica(\'' + l[i].nome + '\')"></td><td class="iconaPlaylist"><label class="switch"  id="global'+ l[i].nome+'"  onclick="pubPriv(\'' + l[i].nome + '\')"><input type="checkbox" checked><span class="slider round"></span><p style="font-size:12px; margin-top:25px;">pubblica</p></label></td><td style="border-top-right-radius:25px; border-bottom-right-radius:25px;" class="iconaPlaylist"><img src="delete.png" class="cuore" title="Cancella Playlist" id="cancella'+l[i].nome+ '" onclick="cancellaplaylist(\'' + l[i].nome + '\')"></td></tr>'
            + '<tr><td colspan="5">'
            + '<div id="aggiungiTag'+l[i].nome+ '"  style=" background-color:white; display:none; margin:0 auto; margin-bottom: 10px; width:800px;">'
            + '<input type="text" class="insTag" id="insTag'+l[i].nome+'" placeholder="#tag">'
            + '<input type="button" class="nascondi" style="float:right; margin-right:32%; height:40px; width:40px; padding:0px;" value="+" onclick="tags(\'' +l[i].nome+ '\')"> </div>'
      ...
      if(eraPubblica){
        ...
        for (let t=0; t<l.length;t++){
          if (l[t].nome==nomeplay && l[t].utente == sessname){
            ...
            pubbliche.push({utente:sessname, nome:nomeplay, id:l[t].id, descrizione:l[t].descrizione, tags:l[t].tags})
            localStorage.setItem('pubbliche', JSON.stringify(pubbliche));
  ...
  //Permette di accedere alla modifica della playlist, ovvero all'aggiunta dei nuovi brani
    ...
    document.getElementById('mostraPlaylist').innerHTML+='<tr><td><table id ="brani'+nome+'" class="canzoni"></table></td></tr>';
    document.getElementById('cerca').style.display="block"
    document.getElementById('canzoni').innerHTML=" "
    document.getElementById('creaplaylist').style.display="none"
    document.getElementById('titoloPagina').innerHTML="Aggiungi Canzoni alla playlist: <br><h1 style='padding:0px;'>" + nome + "</h1>";
    play = nome
  ...
  //Permette di mostrare la preview della canzone  
    ...
    if(tabella.innerHTML==""){
      tabella.innerHTML+= '<link rel="alternate" type="application/json+oembed" href="'+link+'" />'
      +'<iframe id="preview'+canzone+'" style="display:block; margin: 0 auto; border-radius:12px" src="https://open.spotify.com/embed/track/'+canzone+'?utm_source=generator" width="91%" height="80" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>';
  ...
  //Permette di mostrare i risultati di ricerca dato un testo
    ...
    .then((searchResults) =>{ 
      ...
      if(searchResults.tracks.items.length==0){
        ...
        tabella.innerHTML='<tr class="intestazione"><td class="copertina" ></td><td class="titolo">TITOLO</td><td>ARTISTA</td><td>DURATA</td><td> </td></tr><br>'
        for (var i = 0 ; i<searchResults.tracks.items.length; i++){
          var canzone =searchResults.tracks.items[i].id
          var espandi= 'onclick="mostraPreview(\'' + canzone + '\',\'' + searchResults.tracks.items[i].external_urls.spotify + '\')"'
          console.log(searchResults.tracks.items[i])
          tabella.innerHTML= tabella.innerHTML+
         
          '<tr  class="canzone" ><td '+espandi+' class="copertina" ><img class="copertina"  src="'+searchResults.tracks.items[i].album.images[0].url+'"></a></td><td '+espandi+' class="titolo"><h2>'+ searchResults.tracks.items[i].name +
          '</h2></td><td '+espandi+'><h2>'+searchResults.tracks.items[i].album.artists[0].name+
          '</h2></td><td '+espandi+'><h2>'+ calcolaSecondi(searchResults.tracks.items[i].duration_ms) +
          '</h2><td><a><img class = "cuore" src="plus.png" id="add'+canzone+'"  onclick="prefe(\'' + canzone + '\')"> </a></td></tr><tr id ="'+canzone+'"></tr>';
         }
      }
    }
    ).catch(err => {
  ...
  //Cancella una determinata canzone di una determinata playlist
    ...
    if(document.getElementById('bottoneMostraPlaylist').style.display!='none'){
      if(document.getElementById('add'+idsong) != undefined && document.getElementById('add'+idsong) != null  ){
        document.getElementById('add'+idsong).src="plus.png"
      }
    }
        ...
        if (index != -1) {
          ...
          localStorage.setItem('l', JSON.stringify(l))
            ...
            if(l[i].id.length==0){ 
              ...
              tab.innerHTML='<tr><td><p class="nosongs" >Nessuna canzone e\' stata aggiunta alla playlist per il momento</p></td></tr>'
    ...
    if(tab.innerHTML == ""){
      ...
      for(var y =0; y<l.length;y++){
        ...
        if(l[y].nome==nomignolo && l[y].utente==sessname && l[y].id.length>0){
          tab.innerHTML='<tr id="intestazione'+nomignolo+'" class= "intestazione"><td style="width:5%"></td><td class= "titolo">TITOLO</td><td>ARTISTA</td><td>DURATA</td><td></td></tr>'
        ...
        if(l[y].nome==nomignolo && l[y].utente==sessname && l[y].id.length>0){
          ...
          for(var z=0; z<l[y].id.length;z++){
            ...
            .then((response) => response.json())
            .then((songs) =>{ 
              tab.innerHTML+='<tr class="canzone" id="del'+songs.id+'"><td class="copertina"><img class="copertina"  src="'+songs.album.images[0].url+'"></td><td class="titolo">'+songs.name 
              +'</td><td>'+songs.artists[0].name+'</td><td>'+calcolaSecondi(songs.duration_ms)
              +'</td><td><img class = "cuore" src="remove.png"  title="Rimuovi canzone" onclick="deletesong(\'' + songs.id + '\',\'' + nomignolo+'\')"></td></tr>';
            })
          }
          tab.innerHTML+='<br>'
  ...
  for(let i = 0; i<l.length;i++){
      ...
      if(l[i].nome==playlist && l[i].utente==sessname){
        ...
        l[i].tags.forEach(element => {
          ...
          tag.innerHTML+='<div class="tag" id="'+element+playlist+'" >'
          +'<p align="left" style="float: left; margin-top:11px "># '+element+'</p>'
          +'<img style="cursor:pointer; margin-right: -15px; margin-left:7px; margin-top:10px; float:right; height:22px;" onclick="removetag(\'' + element + '\',\'' + playlist+ '\')" src="rimuoviTag.png"></div></td>'
        })
